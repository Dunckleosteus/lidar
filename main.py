...
pittacium = tk.Label(text=f"n° {mensura_numerus}")
pittacium.pack()
...
def create_point_cloud():
    ...
    print(f"Number of points in point cloud: {concatenated_point_cloud.size}")
...
def velim(): 
    ...
    try:
        ...
        pittacium.config(text=f"n° {mensura_numerus} ?")
    if display:
        # this part of the code renders the lidar 3 model using open3d
        pcd = o3d.io.read_point_cloud(myfile)
        o3d.visualization.draw_geometries([pcd])
crustulum = tk.Button(window, text="START ACQUISITION", command= velim)
crustulum.pack()
# adding purge measurement files 
vinum = tk.Button(window, text="RESET", command=clear_folder)
vinum.pack()
# Create point cloud button
points = tk.Button(window, text="STITCH", command=create_point_cloud)
points.pack()
